...
import { clipped, clipShape } from "@expo/ui/swift-ui/modifiers";
...
describe("convertClipped", () => {
  ...
  it("should emit clipShape when overflow hidden and borderRadius present", () => {
    const style = { overflow: "hidden" as const, borderRadius: 8 };
    const result = convertClipped(style);
    expect(result).toEqual(clipShape("roundedRectangle", 8));
  });
});
