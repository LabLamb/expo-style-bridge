...
describe("convertBackgroundColor", () => {
  ...
  it("should pass gradient descriptor through", () => {
    const gradient = {
      gradient: {
    ...
    const result = convertBackgroundColor({ backgroundColor: gradient });
  ...
  it("should pass numeric color through", () => {
    const num = 0xffff0000;
    const result = convertBackgroundColor({ backgroundColor: num });
    expect(result).toEqual(background(num));
  });
});
